...
{   
     ...
     pool.query('SELECT * FROM "posts" ', function (err, result) {
      if (err) {
       ...
       res.send('<html>'+'<head>'+'<title>'+"IAS forum"+'</title>'+'</head>'+'<body>'+'<div background-color="#123456">'+ '<h1 fontcolor="white">'+"IAS Open Forum"+'</h1>'+'</div>'+ '<h2>' +topic+'</h2>' + '<h3>'+"This article was posted by  " +user+'</h3>'+'<h4>'+art+'</h4>'+'</body>'+ '</html>');   
      }
       });
}
else {
